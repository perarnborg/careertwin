...
Dir.glob("./app-sinatra/lib/*.rb").each {|r|
  require r
}

# Loads up all models in path
Dir.glob("./app-sinatra/models/*.rb").each {|r|
  require r
}

# Loads up all controllers in path
Dir.glob("./app-sinatra/controllers/*.rb").each {|r|
  require r
}
...
ActiveRecord::Base.establish_connection(
  :adapter => 'sqlite3',
  :database =>  './app-sinatra/db/careertwin.sqlite3.db'
)
